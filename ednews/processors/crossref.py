...
def crossref_enricher_processor(
    entries: List[dict],
    session=None,
    ...
    for e in entries:
        ...
        try:
            ...
            if entry.get("doi"):
                ...
                doi = (
                    feeds_mod.extract_and_normalize_doi(
                        src, preferred_publication_id=publication_id
                    )
                    if src
                    else None
                )
        ...
        title = (
            entry.get("title") or (entry.get("_entry") or {}).get("title")
            if isinstance(entry.get("_entry"), dict)
            else entry.get("title")
        )

        if not doi and title and feeds_mod.title_suitable_for_crossref_lookup(title):
            try:
                found = cr.query_crossref_doi_by_title(
        ...
        try:
            ...
            try:
                if eddb.article_exists(conn, doi):
                    ...
                    if (
                        check_fields
                        and isinstance(check_fields, (list, tuple))
                        and len(check_fields) > 0
                    ):
                        all_present = True
                        ...
                        all_present = (
                            bool(existing.get("raw"))
                            and bool(existing.get("authors"))
                            and bool(existing.get("abstract"))
                        )
                    ...
                    try:
                        cr = crossref_mod.fetch_crossref_metadata(
                            doi, conn=conn, force=force
                        )
                    except Exception:
                        cr = None
                else:
