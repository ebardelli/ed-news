...
def edworkingpapers_processor(
    html: str,
    base_url: str | None = "https://edworkingpapers.com",
...
def edworkingpapers_feed_processor(
    ...
    entries = edworkingpapers_processor(
        html, base_url=feed_url, publication_id=publication_id
    )
...
def edworkingpapers_postprocessor_db(
    conn,
    feed_key: str,
    entries,
    session=None,
    ...
    try:
        ...
        rows = cur.fetchall()
        items_by_link = {
            r[1]: (r[2] if len(r) > 2 else None) for r in rows if r and r[1]
        }
        items_by_guid = {
            r[0]: (r[2] if len(r) > 2 else None) for r in rows if r and r[0]
        }
    except Exception:
        items_by_link = {}
        items_by_guid = {}
    ...
    try:
        cur.execute(
            "SELECT doi, authors, abstract, published FROM articles WHERE COALESCE(doi, '') != ''"
        )
        rows = cur.fetchall()
        articles_meta = {
            r[0]: {"authors": r[1], "abstract": r[2], "published": r[3]}
            for r in rows
            if r and r[0]
        }
    except Exception:
    ...
    for e in entries:
        try:
            ...
            try:
                ...
                existing_doi = items_by_link.get(link) or items_by_guid.get(
                    e.get("guid")
                )
                    ...
                    if m_abs and m_abs.get("content"):
                        ...
                        if chosen_body:
                            ...
                            found_seed = soup.find(
                                ...
                                and seed in tag.get_text(" ", strip=True)
                            )
                            if found_seed:
                                ...
                                found_anc = None
                                while (
                                    node and getattr(node, "name", None) != "[document]"
                                ):
                                    if getattr(node, "name", None) == "div":
                                        classes = node.get("class") or []
                                        if any(
                                            "field--name-body" in c for c in classes
                                        ) or any("field__item" in c for c in classes):
                                            found_anc = node
                                            break
                                    node = node.parent
                                if found_anc:
                        ...
                        if body_div:
                            ...
                            time_el = soup.select_one(
                                "div.field--name-field-wp-date time"
                            )
                            if time_el:
                            ...
                            if not abstract:
                                found = soup.find(
                                    lambda tag: tag.name in ("p", "div")
                                    and len(tag.get_text(" ", strip=True)) > 50
                                )
                                if found:
