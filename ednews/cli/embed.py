...
def cmd_embed(args: Any) -> None:
        ...
        try:
            ...
            if ids_arg:
                try:
                    ...
                    ids = []
                if ids:
                    embeddings.generate_and_insert_embeddings_for_ids(
                        conn, ids, model=args.model
                    )
                    # still generate missing ones for others if force is set
                    if force:
                        embeddings.generate_and_insert_article_embeddings(
                            conn,
                            model=args.model,
                    ...
                    embeddings.generate_and_insert_embeddings_local(
                        conn, model=args.model, batch_size=args.batch_size
                    )
            else:
                if force:
                    embeddings.generate_and_insert_article_embeddings(
                        conn, model=args.model, batch_size=args.batch_size, force=True
                    )
                else:
                    embeddings.generate_and_insert_embeddings_local(
                        conn, model=args.model, batch_size=args.batch_size
    ...
    if want_headlines:
        try:
            ...
            embeddings.generate_and_insert_headline_embeddings(
                conn, model=args.model, batch_size=args.batch_size
            )
        except Exception:
