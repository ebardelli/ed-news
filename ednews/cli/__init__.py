...
def run() -> None:
    ...
    p_fetch.add_argument(
        "--articles",
        action="store_true",
        help="Also fetch article feeds (default: both articles and headlines if no flags are set)",
    )
    p_fetch.add_argument(
        "--headlines",
        action="store_true",
        help="Also fetch news headlines (default: both articles and headlines if no flags are set)",
    )
    p_fetch.add_argument(
        "--debug-feed",
        help="Debug a single feed by key or URL and print step-by-step output to the terminal",
    )
    ...
    p_embed.add_argument(
        "--headlines",
        action="store_true",
        help="Also generate embeddings for news headlines",
    )
    p_embed.add_argument(
        "--articles",
        action="store_true",
        help="Generate embeddings for articles (default: both articles and headlines if no flags are set)",
    )
    p_embed.add_argument(
        "--force",
        action="store_true",
        help="Force re-generation of embeddings for existing articles",
    )
    p_embed.add_argument(
        "--ids",
        type=str,
        default=None,
        help="Comma-separated list of article ids to (re)embed",
    )
    p_embed.set_defaults(func=cmd_embed)

    p_issn = sub.add_parser(
        "issn-lookup", help="Fetch latest works for journals by ISSN and insert into DB"
    )
    p_issn.add_argument(
        "--per-journal",
        type=int,
        default=30,
        help="Number of works to fetch per journal (uses cursor pagination; no hard max)",
    )
    p_issn.add_argument(
    ...
    p_issn.add_argument(
        ...
        type=str,
        default="created",
        help="Field to sort by when requesting works (e.g. created, deposited)",
    )
    p_issn.add_argument(
        "--date-filter-type",
        type=str,
        choices=["created", "updated", "indexed"],
        default=None,
        help="Use Crossref date filters (from-*/until-*)",
    )
    p_issn.add_argument(
        "--from-date",
        type=str,
    ...
    p_headlines = sub.add_parser(
        "headlines", help="Fetch latest headlines from news.json sites"
    )
    p_headlines.add_argument("--out", help="Write output JSON to this file")
    p_headlines.add_argument(
        "--no-persist",
        action="store_true",
        help="Do not persist fetched headlines to the configured DB (default: persist)",
    )
    ...
    p_cleanup = manage_sub.add_parser(
        "cleanup-empty-articles", help="Remove articles with no title and no abstract"
    )
    p_cleanup.add_argument(
        "--older-than-days",
        type=int,
        default=None,
        help="Only delete articles older than this many days (based on fetched_at or published)",
    )
    p_cleanup.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not delete; only report how many rows would be deleted",
    )
    p_cleanup.set_defaults(func=lambda args: cmd_manage_db_cleanup(args))

    p_cleanup_ft = manage_sub.add_parser(
        "cleanup-filtered-title",
        help="Remove articles whose titles match configured filters",
    )
    p_cleanup_ft.add_argument(
        "--filter", action="append", help="A single title to filter (repeatable)"
    )
    p_cleanup_ft.add_argument(
        "--filters",
        type=str,
        default=None,
        help="Comma-separated list of titles to filter",
    )
    p_cleanup_ft.add_argument(
        "--dry-run",
        action="store_true",
    ...
    p_migrate = manage_sub.add_parser(
        "migrate", help="Run schema migrations (no-op placeholder)"
    )
    p_migrate.set_defaults(func=lambda args: cmd_manage_db_migrate(args))

    p_sync = manage_sub.add_parser(
        "sync-publications", help="Sync publications table from feeds list"
    )
    p_sync.set_defaults(func=lambda args: cmd_manage_db_sync_publications(args))

    p_rematch = manage_sub.add_parser(
        "rematch-dois",
        help="Clear DOIs for a publication or feeds and re-run Crossref matching",
    )
    p_rematch.add_argument(
        "--publication-id",
        help="Publication ID to target (will resolve feeds from publications table)",
    )
    p_rematch.add_argument(
        "--feed",
        action="append",
        help="Feed key to target (repeatable). If omitted, publication-id will be used to resolve feeds",
    )
    p_rematch.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not modify DB; only report what would be done",
    )
    p_rematch.add_argument(
    ...
    p_remove = manage_sub.add_parser(
        ...
        help="Remove articles associated with specific feeds or a publication",
    )
    p_remove.add_argument(
        "--publication-id",
        help="Publication ID to target (will match articles.publication_id)",
    )
    p_remove.add_argument(
        "--feed",
        action="append",
        help="Feed key to target (repeatable). Matches articles.feed_id",
    )
    p_remove.add_argument(
        "--dry-run",
        action="store_true",
    ...
    p_runall = manage_sub.add_parser(
        "run-all",
        help="Run migrations, sync publications, cleanup, and vacuum in sequence",
    )
    p_runall.add_argument(
        "--older-than-days", type=int, default=None, help="Pass-through to cleanup step"
    )
    p_runall.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not perform destructive actions; only report",
    )
    p_runall.set_defaults(func=lambda args: cmd_manage_db_run_all(args))

    p_serve = sub.add_parser(
        "serve", help="Serve the built static site from the build directory"
    )
    ...
    p_post = sub.add_parser(
        "postprocess", help="Run a DB-level postprocessor (e.g. crossref) for feeds"
    )
    p_post.add_argument(
        "--processor",
        required=False,
        help="Name of the processor to run (e.g. crossref). If omitted, per-feed configured processors will be used when present; crossref is used as a final fallback.",
    )
    p_post.add_argument(
        "--feed",
        action="append",
        help="Feed key to limit processing to (repeatable). If omitted, feeds are auto-detected from feeds list",
    )
    p_post.add_argument(
        "--only-missing",
        action="store_true",
        help="Only include items where the specified field is missing/empty (default: doi)",
    )
    p_post.add_argument(
        "--missing-field",
        type=str,
        default="doi",
        help="Field to check for missingness when --only-missing is set (default: doi)",
    )
    p_post.add_argument(
        "--force",
        action="store_true",
        help="Force re-fetch even when existing metadata appears present",
    )
    p_post.add_argument(
        "--check-fields",
        type=str,
        default=None,
        help="Comma-separated list of article fields to require before skipping (e.g. raw,authors,abstract). If omitted, processors choose their defaults.",
    )
