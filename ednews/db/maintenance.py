...
def rematch_publication_dois(
    ...
    if not keys:
        ...
        if not keys:
            logger.debug(
                "rematch_publication_dois: no feed keys resolved for publication_id=%s feed_keys=%s",
                publication_id,
                feed_keys,
            )
            return results
    ...
    for fk in keys:
        try:
            ...
            if only_wrong and not wrong_items:
                try:
                    logger.info(
                        "rematch_publication_dois: feed=%s no wrong items; skipping postprocessor (only-wrong)",
                        fk,
                    )
                except Exception:
                    pass
            ...
            if only_wrong and retry_limit and retry_limit > 0:
                try:
                    ...
                    if skipped_guids:
                        logger.info(
                            "rematch_publication_dois: feed=%s skipping %d guids that exceeded retry_limit=%s",
                            fk,
                            len(skipped_guids),
                            retry_limit,
                        )
            ...
            if only_wrong and not to_process:
                try:
                    logger.info(
                        "rematch_publication_dois: feed=%s no items to process after retry filtering; skipping postprocessor",
                        fk,
                    )
                except Exception:
                    pass
            ...
            if post_fn_for_feed:
                ...
                try:
                    ...
                    try:
                        ...
                        sample_guids = None
                    logger.debug(
                        "rematch_publication_dois: calling postprocessor for feed=%s entries=%s sample_guids=%s",
                        fk,
                        len(entries),
                        sample_guids,
                    )
                    ...
                    logger.info(
                        "rematch_publication_dois: postprocessor returned %s updates for feed=%s",
                        updated or 0,
                        fk,
                    )
                    ...
                    try:
                        guids_to_check = [
                            e.get("guid") for e in entries if e.get("guid")
                        ]
                        if guids_to_check:
                            ...
                            logger.debug(
                                "rematch_publication_dois: postprocessor result for feed=%s item_dois=%s",
                                fk,
                                post_map,
                            )
                            # If postprocessor reported zero updates but DOIs remain empty, warn
                            if (not updated) and any(
                                (not v) for v in post_map.values()
                            ):
                                logger.warning(
                            ...
                            try:
                                ...
                                logger.exception(
                                    "Failed to update rematch_attempts after postprocessor for feed=%s",
                                    fk,
                                )
                    except Exception:
                        logger.exception(
                            "Failed to re-query items after postprocessor for feed=%s",
                            fk,
                        )
...
def remove_feed_articles(
    ...
    try:
        ...
        if not keys:
            logger.debug(
                "remove_feed_articles: no feed keys resolved for publication_id=%s feed_keys=%s",
                publication_id,
                feed_keys,
            )
            return 0
                ...
                if expected_pub:
                    ...
                    try:
                        if dry_run:
                            ...
                            logger.info(
                                "remove_feed_articles dry-run feed=%s would delete %d/%d rows (expected_pub=%s)",
                                fk,
                                to_delete,
                                total_with_doi,
                                expected_pub,
                    ...
                    if dry_run:
                        ...
                        cnt = row[0] if row and row[0] else 0
                        logger.info(
                            "remove_feed_articles dry-run feed=%s has no publication_id; would delete %d rows",
                            fk,
                            cnt,
                        )
                        total_deleted += cnt
